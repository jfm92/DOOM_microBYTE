...
#define COLORPALXORIG ((240 - 16*(CHIP_SIZE+1))/2)
#define COLORPALYORIG ((240 - 16*(CHIP_SIZE+1))/2)
...
static void M_DrawSetting(const setup_menu_t* s)
{
  ...
  if (flags & S_COLOR) // Automap paint chip
    {
      ...
      // draw the paint chip
      V_FillRect(0, x*SCREENWIDTH/240, (y-1)*SCREENHEIGHT/240,
                    8*SCREENWIDTH/240, 8*SCREENHEIGHT/240,
                 PAL_BLACK);
      V_FillRect(0, (x+1)*SCREENWIDTH/240, y*SCREENHEIGHT/240,
                        6*SCREENWIDTH/240, 6*SCREENHEIGHT/240,
                 (byte)ch);
  ...
  char_width = 7; // default for end of line
      ...
      V_FillRect(0, ((x+cursor_start-1)*SCREENWIDTH)/240, (y*SCREENHEIGHT)/240,
      (char_width*SCREENWIDTH)/240, 9*SCREENHEIGHT/240, PAL_WHITE);
    }
