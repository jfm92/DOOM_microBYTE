                ...
                enum patch_translation_e flags)
{
  ...
  {
    srcx=srcx*SCREENWIDTH/240;
    srcy=srcy*SCREENHEIGHT/240;
    width=width*SCREENWIDTH/240;
    height=height*SCREENHEIGHT/240;
    destx=destx*SCREENWIDTH/240;
    desty=desty*SCREENHEIGHT/240;
  }
        ...
        int cm, enum patch_translation_e flags)
{
  ...
  if (flags & VPT_STRETCH)
    if ((SCREENWIDTH==240) && (SCREENHEIGHT==240))
      flags &= ~VPT_STRETCH;
  ...
  if (V_GetMode() == VID_MODE8 && !(flags & VPT_STRETCH)) {
    ...
    if (y<0 || y+patch->height > ((flags & VPT_STRETCH) ? 240 :  SCREENHEIGHT)) {
    ...
    int   DX  = (SCREENWIDTH<<16)  / 240;
    int   DXI = (240<<16)          / SCREENWIDTH;
    int   DY  = (SCREENHEIGHT<<16) / 240;
    int   DYI = (240<<16)          / SCREENHEIGHT;
