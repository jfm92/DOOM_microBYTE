...
void WI_drawLF(void)
{
  ...
  V_DrawNamePatch((240 - V_NamePatchWidth(lname))/2, y,
  ...
  // CPhipps - patch drawing updated
  V_DrawNamePatch((240 - V_NamePatchWidth(finished))/2, y,
...
void WI_drawEL(void)
{
  ...
  // draw "Entering"
  // CPhipps - patch drawing updated
  V_DrawNamePatch((240 - V_NamePatchWidth(entering))/2,
  ...
  // CPhipps - patch drawing updated
  V_DrawNamePatch((240 - V_NamePatchWidth(lname))/2, y, FB,
...
static void WI_drawTimeStats(int cnt_time, int cnt_total_time, int cnt_par)
{
  ...
  WI_drawTime(240/2 - SP_TIMEX, SP_TIMEY, cnt_time);

  V_DrawNamePatch(SP_TIMEX, (SP_TIMEY+240)/2, FB, total, CR_DEFAULT, VPT_STRETCH);
  WI_drawTime(240/2 - SP_TIMEX, (SP_TIMEY+240)/2, cnt_total_time);
  ...
  if (!(modifiedgame && !deh_pars))
  {
    if (wbs->epsd < 3)
    {
      V_DrawNamePatch(240/2 + SP_TIMEX, SP_TIMEY, FB, par, CR_DEFAULT, VPT_STRETCH);
      WI_drawTime(240 - SP_TIMEX, SP_TIMEY, cnt_par);
    }
  }
}
...
void WI_drawStats(void)
{
  ...
  if (cnt_kills)
    WI_drawPercent(240 - SP_STATSX, SP_STATSY, cnt_kills[0]);

  V_DrawNamePatch(SP_STATSX, SP_STATSY+lh, FB, items, CR_DEFAULT, VPT_STRETCH);
  if (cnt_items)
    WI_drawPercent(240 - SP_STATSX, SP_STATSY+lh, cnt_items[0]);

  V_DrawNamePatch(SP_STATSX, SP_STATSY+2*lh, FB, sp_secret, CR_DEFAULT, VPT_STRETCH);
  if (cnt_secret)
    WI_drawPercent(240 - SP_STATSX, SP_STATSY+2*lh, cnt_secret[0]);
