...
static void R_InitLightTables (void)
{
  ...
  for (i=0; i< LIGHTLEVELS; i++)
    ...
    //           brighter than normal res
          int scale = FixedDiv ((240/2*FRACUNIT), (j+1)<<LIGHTZSHIFT);
...
// proff 11/06/98: Added for high-res
  projectiony = ((SCREENHEIGHT * centerx * 240) / 240) / SCREENWIDTH * FRACUNIT;
  ...
  // psprite scales
// proff 08/17/98: Changed for high-res
  pspritescale = FRACUNIT*viewwidth/240;
  pspriteiscale = FRACUNIT*240/viewwidth;
// proff 11/06/98: Added for high-res
  pspriteyscale = (((SCREENHEIGHT*viewwidth)/SCREENWIDTH) << FRACBITS) / 240;
